<?php
  ...
  if(ISSET($_POST['search'])) {
    ...
    COUNT(case when tb1.id_tipo_documento='3' and tb1.id_sucursal=tb1.id_sucursal then 1 else null end) + COUNT(case when tb1.id_tipo_documento='1' then 1 else null end) AS total 
    FROM tb_facturacion_cab tb1 
...
<?php
    ...
    INNER JOIN tb_sucursales tb3 ON tb1.id_sucursal=tb3.id_sucursal 
    INNER JOIN tb_encomienda_cab tb4 on tb4.id_encomienda=tb1.id_facturacion and tb4.id_pago = 1 
    GROUP BY tb2.nombre_usuario ORDER BY tb2.nombre_usuario") or die(mysqli_error($con));
    while($fetch=mysqli_fetch_array($query)) {
?>
  <tr>
    <td><?php echo $fetch['nombre_sucursal']?></td>
